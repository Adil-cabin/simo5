...
export const formatters = {
  patientNumber: (number: string | undefined | null, status?: 'pending' | 'validated') => {
    if (!number) return '-';
    
    // If status is provided, use it to determine the prefix
    if (status) {
    ...
    // Otherwise, preserve the existing prefix
    const numericPart = number.replace(/\D/g, '');
    const prefix = number.startsWith('PA') ? 'PA' : 'P';
    return `${prefix}${numericPart.padStart(4, '0')}`;
  },
...
export const validateFormatters = {
  ...
  patientNumber: (number: string | undefined | null) => {
    ...
    return /^P\d{4}$/.test(number) || /^PA\d{4}$/.test(number);
  }
};